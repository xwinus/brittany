module StandaloneDerivingSpec (spec) where
...
spec projectRoot = Hspec.describe "standalone deriving declarations" $ do
  formattingExampleAt 80 projectRoot
    "formats the Headroom deriving-via declaration natively"
    "StandaloneDerivingInput.hs"
    "StandaloneDerivingExpected.hs"
  formattingExampleAt 80 projectRoot
    "preserves strategies, binders, contexts, and overlap pragmas"
    "StandaloneDerivingEdgeInput.hs"
    "StandaloneDerivingEdgeExpected.hs"
  formattingExampleAt 40 projectRoot
    "wraps standalone deriving declarations at narrow width"
    "StandaloneDerivingEdgeInput.hs"
    "StandaloneDerivingEdgeNarrow.hs"
  exactCommentExample projectRoot
    "preserves ambiguous comments through declaration fallback"
    "StandaloneDerivingCommented.hs"
  parseFailureExample projectRoot
    "rejects incomplete standalone deriving without changing input"
    "StandaloneDerivingInvalid.hs"
...
formattingExampleAt columns projectRoot description inputName expectedName =
  Hspec.it description $ do
    let input = fixturePath projectRoot inputName
        ...
        args = ["--columns", show columns, "--fail-on-fallback"]
...
exactCommentExample projectRoot description fixtureName =
  Hspec.it description $ do
    let fixture = fixturePath projectRoot fixtureName
        output = outputPath projectRoot fixtureName
        args = formatterArgs projectRoot output
